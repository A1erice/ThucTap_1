using System;
using System.Collections.Generic;

namespace ThucTap_1.Models;

public partial class BkKtcc
{
    public int Id { get; set; }

    public DateTime? NgayKt { get; set; }

    public string? NguoiKt { get; set; }

    public string? NguoiDuocKt { get; set; }

    public string? KhoaKt { get; set; }

    public string? SoHs { get; set; }

    public double? A11 { get; set; }

    public double? A12 { get; set; }

    public double? A13 { get; set; }

    public double? A21 { get; set; }

    public double? A22 { get; set; }

    public double? A23 { get; set; }

    public double? B3 { get; set; }

    public double? B4 { get; set; }

    public double? B5 { get; set; }

    public double? B6a { get; set; }

    public double? B6b { get; set; }

    public double? B7 { get; set; }

    public double? C81 { get; set; }

    public double? C82 { get; set; }

    public double? C9 { get; set; }

    public double? C10 { get; set; }

    public double? C11 { get; set; }

    public double? TongDiem { get; set; }

    public string? XepLoai { get; set; }

    public string? NhanXet { get; set; }
}
